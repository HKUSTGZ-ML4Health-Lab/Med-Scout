...
class MedScoutBaseORM(ORM):
    ...
    def _identify_task(self, solution: str) -> str:
        ...
        if "Label" in solution:
            return "scale_size"
        elif "(" in solution and ")" in solution:
            return "scale_pos"
        elif " " in solution and "," not in solution:
            if re.match(r'^[\d\s]+$', solution):
                return "shuffle"
            return "unknown"
        else:
...
class MedScoutFormatORM(MedScoutBaseORM):
    ...
    def __call__(self, completions: List[str], solution: List[str], **kwargs) -> List[float]:
        ...
        for completion, sol in zip(completions, solution):
            pred_content = self._extract_answer_content(completion)
            sol_content = self._extract_answer_content(sol)
            ...
            try:
                ...
                if task_type == "replace":
                    ...
                    if not sol_coords: score = 0.0
                        ...
                        s3 = 0.0
                        if s2 > 0 and all(x1 <= x2 and y1 <= y2 for x1, y1, x2, y2 in comp_coords):
                            s3 = 1/3
                        score = s1 + s2 + s3
        ...
        for completion, sol in zip(completions, solution):
            ...
            try:
                ...
                if task_type == "replace":
                    ...
                    if is_valid_format:
                        if len(sol_seq) <= 2:
                            score = 1.0 if comp_seq == sol_seq else 0.0
                        else:
                            correct = sum(1 for c, s in zip(comp_seq, sol_seq) if c == s)
                            score = correct / len(sol_seq)
            except Exception:
                score = 0.0
